...
$\mathbf{G}_{Del}$은 Delaunay triangulation을 위해 샘플링된 점이며  $f_{\mu_q}$는 Gaussian의 center 입니다. 이항은 Gaussian의 encer를 surface 안쪽으로 포함시키도록 유도하며 SDF values가 음수가 되도록 유도합니다. $L_{\mathrm{erosion}}$은 center에만 적용되어 mesh의 붕괴를 방지합니다.
...
H 는 cross-entropy loss, $\sigma$ 는 sigmoid function, $f_{\mathrm{p}}$는 Delaunay site p의 SDF values, $o_p$는 p의 occupancy label을 의미합니다.

...
여기서 $⊮[\cdot]$는 indicator function이며  $\mathbf{u}^{(j)}_i$ 는 카메라 $j$로부터 가우시안 $g_i$를 투영하는 픽셀 좌표입니다. 그래서 whitelist에 포함하는 모든 Gaussian은 1로 간주합니다. Gaussian이 어떠한 masked view에도 투영되지 못했다면 background에 해당하므로 지웁니다.

...
$\mathbf{u} = [u, v]^{\top}$ 은 pixel coordinates이며 $d$은 depth 를 의미합니다. 만약 $d \le 0$ 이거나 $u$ 가 image 밖이라면 projection을 하지 않습니다. 여기서 $x_{cam}$ 은 카메라 좌표계에서 본 3D 점 $x$를 의미하며 $[2]$는 z축을 의미합니다. 
